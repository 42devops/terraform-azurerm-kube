...
resource "azurerm_virtual_machine" "kubernetes_master" {
  ...
  location              = var.location
  resource_group_name   = var.resource_group_name
  network_interface_ids = list(element(azurerm_network_interface.kubernetes_network_interface.*.id, count.index))
...
resource "azurerm_virtual_machine" "kubernetes_minion" {
  ...
  location              = var.location
  resource_group_name   = var.resource_group_name
  network_interface_ids = list(element(azurerm_network_interface.kubernetes_network_interface.*.id, count.index + var.kube_master_count))
